...
class DPEngineCoreProc(EngineCoreProc):
    ...
    def _maybe_publish_request_counts(self):
        ...
        if counts != self.last_counts:
            ...
            if envs.VLLM_DP_TRACE:
                logger.info(
                    "[dp-trace] publish counts running=%d waiting=%d "
    ...
    def run_busy_loop(self):
        ...
        while self._handle_shutdown():
            ...
            if not self.engines_running:
                if envs.VLLM_DP_TRACE:
                    counts = self.scheduler.get_request_counts()
                    # If outputs are still queued or requests still tracked
                    # at sleep time, the engine is pausing with undelivered
                    # work. Log that state before publishing wave_complete.
                    try:
                        _oq = self.output_queue.qsize()
                    except Exception:
                        _oq = -1
                    logger.info(
                        "[dp-trace] SLEEP wave=%d step=%d running=%d waiting=%d "
